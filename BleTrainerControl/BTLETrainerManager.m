...
{
    //Scan for primary service of cycleops powerbeam bluetooth
    [manager startScanningWithServicesUUIDArray:[NSArray arrayWithObjects:
             [CBUUID UUIDWithString:TACX_FEC_PRIMARY_SERVICE],
             [CBUUID UUIDWithString:TACX_VORTEX_PRIMARY_SERVICE],
             nil
         ]
     ];
}
...
-(void)dataReceivedFromPeripheral:(CBPeripheral *)peripheral withCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    else
    {
        ...
        [self dataReceivedByCharacteristic:cbuuid data: data error:error];
    }
}
...
-(void)dataReceivedByCharacteristic:(CBUUID *)characteristic data:(NSData *)data error:(NSError *)error
{
    ...
    if([characteristic respondsToSelector:@selector(UUIDString)])
        characteristicUUID = [characteristic UUIDString];
    else
        characteristicUUID = [[[NSString alloc] initWithData:characteristic.data encoding:NSUTF8StringEncoding] autorelease];
                    ...
                    if([hexaString length] >= 8 + 2 * dataLength)
                    {
                        ...
                        if([dataHexString length] == 16)
                        {
                            ...
                            if(pageNumber == 2)
                            {
                                ...
                                speedCondition = [utils getDecimalFromBinary: [calibrationConditionsBinaryString substringWithRange:NSMakeRange(0, 2)]];
                                temperatureCondition = [utils getDecimalFromBinary: [calibrationConditionsBinaryString substringWithRange:NSMakeRange(2, 2)]];
                            ...
                            if(pageNumber == 51)
                            {
                                ...
                                NSString *rollingResistanceHexaString = [dataHexString substringWithRange:NSMakeRange(14, 2)];
