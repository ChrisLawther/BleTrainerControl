...
extension Data {
    ...
    func checksummed() ->Data {
        var result = self
        let checksum = self.reduce(0) { $0 ^ $1 } // { (checksum, value) in return checksum ^ value }
...
enum FECMessage {
    ...
    private func messageBody() throws -> Data {
        ...
        case .windResistanceCoefficient(let kgMValue, let windspeed, let draftingFactor):
            return try windResistanceCoefficientData(kgmValue: kgMValue, windspeed: windspeed, draftingFactor: draftingFactor)
    ...
    private func windResistanceCoefficientData(kgmValue: Float, windspeed: Float, draftingFactor: Float) throws -> Data {
